...
import com.survey.dataEntry.model.TubeWellUserDataModel;
...
public class TubeWellUserDataController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        TubeWellUserDataModel tubeWellUserDataModel = new TubeWellUserDataModel();
        tubeWellUserDataModel.setDriverClass(ctx.getInitParameter("driverClass"));
        tubeWellUserDataModel.setConnectionString(ctx.getInitParameter("connectionString"));
        tubeWellUserDataModel.setDb_username(ctx.getInitParameter("db_username"));
        tubeWellUserDataModel.setDb_password(ctx.getInitParameter("db_password"));
        tubeWellUserDataModel.setConnection();
        ...
        try {
            ...
            if (JQstring != null) {
                ...
                if (JQstring.equals("getTubeWellUserType")) {
                    list = tubeWellUserDataModel.getTubeWellUserType(q);
                } else if (JQstring.equals("getStatusType")) {
                    list = tubeWellUserDataModel.getStausType(q);
                }
                ...
                tubeWellUserDataModel.closeConnection();
                return;
            }
        } catch (Exception e) {
            ...
            while (itr.hasNext()) {
                ...
                if (item.isFormField()) {
                    ...
                    if (item.getName() == null || item.getName().isEmpty()) {
                        ...
                        destination_path = tubeWellUserDataModel.getRepositoryPath(image_uploaded_for);
...
//                        }
        ...
        if (task1.equals("generateMapReport")) {
            ...
            listAll = tubeWellUserDataModel.showAllData(searchTubeWellUser);
            jrxmlFilePath = ctx.getRealPath("/report/tubeWellUSer.jrxml");
            byte[] reportInbytes = tubeWellUserDataModel.generateMapReport(jrxmlFilePath, listAll);
        ...
        if (task.equals("Delete")) {
            tubeWellUserDataModel.deleteRecord(Integer.parseInt(map.get("tubewell_user_id")));  // Pretty sure that organisation_type_id will be available.
            ...
            TubeWellUserDataTypeBean tubeWellUserDataTypeBean = new TubeWellUserDataTypeBean();
            tubeWellUserDataTypeBean.setTubewell_user_id(tubewell_user_id);
            //   tubeWellUserDataTypeBean.setTubewell_user_id(Integer.parseInt(map.get("tubewell_user_id")));
            tubeWellUserDataTypeBean.setTubewell_user_id(tubeWellUserDataModel.getTubeWellUserId(map.get("tubewell_user")));
            tubeWellUserDataTypeBean.setStatus_id(tubeWellUserDataModel.getStatusId(map.get("status_type")));
            tubeWellUserDataTypeBean.setReading(map.get("reading"));
            tubeWellUserDataTypeBean.setLattitude(map.get("lattitude"));
            tubeWellUserDataTypeBean.setLongitude(map.get("longitude"));
            tubeWellUserDataTypeBean.setRemark(map.get("user_name"));
            tubeWellUserDataTypeBean.setImage_name(map.get("image_name"));
            if (tubewell_user_id == 0) {
                System.out.println("Inserting values by model......");
                tubeWellUserDataModel.insertRecord(tubeWellUserDataTypeBean, list);
            } else {
                System.out.println("Update values by model........");
                tubeWellUserDataModel.updateRecord(tubeWellUserDataTypeBean);
            }
        }
        ...
        noOfRowsInTable = tubeWellUserDataModel.getNoOfRows(searchTubeWellUser);
        ...
        if (task1.equals("viewImage")) {
            try {
                ...
                String destination = tubeWellUserDataModel.getdestinationPath(general_image_details_id, image_name);
        ...
        List<TubeWellUserDataTypeBean> tubeWellUserTypeList = tubeWellUserDataModel.showData(lowerLimit, noOfRowsToDisplay, searchTubeWellUser);
        ...
        // End of Search Table
        System.out.println("color is :" + tubeWellUserDataModel.getMsgBgColor());
        request.setAttribute("IDGenerator", new UniqueIDGenerator());
        request.setAttribute("searchTubeWellUser", searchTubeWellUser);
        request.setAttribute("message", tubeWellUserDataModel.getMessage());
        request.setAttribute("msgBgColor", tubeWellUserDataModel.getMsgBgColor());
        request.getRequestDispatcher("/tubeWellUserDataView").forward(request, response);
