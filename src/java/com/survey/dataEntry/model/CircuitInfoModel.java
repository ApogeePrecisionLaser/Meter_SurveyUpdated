...
import com.survey.dataEntry.controller.CircuitInfoBean;
...
public class CircuitInfoModel {
    ...
    public int getNoOfRows(String parentheadername, String subheadername, String headerindex) {
        ...
        String query = "SELECT count(t.circuitinfo_id)"
                + "FROM circuit_info as n,tree as t, circuit_info as pn "
                + "where n.circuitinfo_id=t.circuitinfo_id AND t.circuitinfo_parent_id=pn.circuitinfo_id "
                + "And n.active='Y' And t.active='Y' and pn.active='Y' AND t.index_no NOT LIKE '0%' " //t.index_no NOT LIKE '%.0%' AND t.index_no NOT LIKE '0%'
                + " AND if('" + subheadername + "'='', n.circuit_name LIKE '%%', n.circuit_name LIKE '%" + subheadername + "%')"
                + " AND if('" + parentheadername + "'='', pn.circuit_name LIKE '%%', pn.circuit_name LIKE '%" + parentheadername + "%')"
...
//         noOfRows = noOfRows+1;
    ...
    public List<CircuitInfoBean> showData(int lowerLimit, int noOfRowsToDisplay, String parentheadername, String subheadername, String headerindex) {
        List<CircuitInfoBean> list = new ArrayList<CircuitInfoBean>();
...
//        /* code to show root node -- ends here */
        ...
        String query = "SELECT t.index_no,t.circuitinfo_id,n.circuit_name, pn.circuit_name AS parent_circuit_name,t.active,t.remark "
                + " FROM circuit_info as n,tree as t, circuit_info as pn "
                + " where n.circuitinfo_id=t.circuitinfo_id AND t.circuitinfo_parent_id=pn.circuitinfo_id "
        ...
        try {
            ResultSet rset = connection.prepareStatement(query).executeQuery();
            while (rset.next()) {
                CircuitInfoBean headerNode = new CircuitInfoBean();
                headerNode.setIndex_no(rset.getString("index_no"));
                headerNode.setNode_id(rset.getInt("circuitinfo_id"));
                headerNode.setTree_node_name(rset.getString("circuit_name"));
                headerNode.setNode_parent_name(rset.getString("parent_circuit_name"));
    ...
    public List<CircuitInfoBean> showDataParent(int lowerLimit, int noOfRowsToDisplay, String headername, String headerindex) {
        List<CircuitInfoBean> list = new ArrayList<CircuitInfoBean>();
        String addQuery = " LIMIT " + lowerLimit + ", " + noOfRowsToDisplay;
        if (lowerLimit == -1) {
            addQuery = "";
        ...
        // int nodeId = 1;
        String query1 = "SELECT n.circuitinfo_id,n.circuit_name "  
                + " FROM circuit_info as n "  
                + " where n.active='Y' "  
                + " AND if('" + headername + "'='', n.circuit_name LIKE '%%', n.circuit_name LIKE '%" + headername + "%')"
        ...
        String query = "SELECT t.index_no,t.circuitinfo_id,n.circuit_name, pn.circuit_name AS parent_node_name,t.active,t.remark "
                + "FROM circuit_info as n,tree as t, circuit_info as pn "
                + "where n.circuitinfo_id=t.circuitinfo_id AND t.circuitinfo_parent_id=pn.circuitinfo_id "
                + "And n.active='Y' And t.active='Y' and pn.active='Y' AND t.index_no NOT LIKE '0%' " //index_no not like '%0%'
                + " AND if('" + headername + "'='', n.circuit_name LIKE '%%', n.circuit_name LIKE '%" + headername + "%')"
    ...
    public List<CircuitInfoBean> showChild_Header_Name(String headername, String headerindex, String parentHeaderName) {
        List<CircuitInfoBean> list = new ArrayList<CircuitInfoBean>();
        int genNo = 0, maxGenNo = 0, parentHeaderId = 0;

        try {
            parentHeaderId = getNodeId(parentHeaderName);
            String query1 = "SELECT generation_no FROM circuit_info where active='Y' and circuitinfo_id = " + parentHeaderId;
            String query2 = "SELECT max(generation_no) as generation_no FROM circuit_info where active='Y' ";
            ...
            String query3 = "SELECT t.index_no,t.circuitinfo_id,n.circuit_name from circuit_info As n,tree as t where n.generation_no Between " + genNo + " And " + maxGenNo
                    + " and t.circuitinfo_id NOT IN (SELECT circuitinfo_id FROM tree where active='Y' AND circuitinfo_parent_id = " + parentHeaderId + ") " //  index_no LIKE '%.0%' AND index_no LIKE '0%' Or //AND index_no NOT LIKE '%0%' and  //AND index_no LIKE '%0%'
                    + " AND if('" + headername + "'='', n.circuit_name LIKE '%%', n.circuit_name LIKE '%" + headername + "%')"
                    + " AND if('" + headerindex + "'='', t.index_no LIKE '%%',t.index_no LIKE '" + headerindex + "%') "
                    + " AND t.circuitinfo_id=n.circuitinfo_id And t.active='Y' And n.active='Y' " //AND t.index_no NOT LIKE '%0%'
                    + " and t.circuitinfo_id != " + parentHeaderId + "  ORDER BY n.circuitinfo_id";    // " ORDER BY CAST(SUBSTRING_INDEX(t.index_no, '.', 1) AS UNSIGNED),CAST(SUBSTRING_INDEX(t.index_no, '.', -1) AS UNSIGNED),t.index_no "
...
//            }
    ...
    public List<CircuitInfoBean> showData1(int lowerLimit, int noOfRowsToDisplay, String headername, String headerindex) {
        List<CircuitInfoBean> list = new ArrayList<CircuitInfoBean>();
        String addQuery = " LIMIT " + lowerLimit + ", " + noOfRowsToDisplay;
        if (lowerLimit == -1) {
            addQuery = "";
    ...
    public List<String> getIndex(String q) {
        ...
        int nodeId = 1;
        String query1 = "SELECT index_no from tree where active='Y' and circuitinfo_id != " + nodeId // + " AND index_no NOT LIKE '%.0%' AND index_no NOT LIKE '0%' " //index_no not like '%0%'
                + " ORDER BY CAST(SUBSTRING_INDEX(t.index_no, '.', 1) AS UNSIGNED),CAST(SUBSTRING_INDEX(t.index_no, '.', -1) AS UNSIGNED),t.index_no "; //ORDER BY t.index_no
    ...
    public boolean checkNodeName(String NodeName) {
        boolean check = false;
        int count = 0;
        String query = "SELECT count(*) FROM circuit_info WHERE circuit_name = ? ";
        try {
    ...
    public int insertNodeRecord(CircuitInfoBean headerNode, int NodeParentId, String NodeName) {
        String query = "INSERT INTO circuit_info(circuit_name, generation_no, active, revision_no, remark) VALUES(?,?,?,?,?) ";
        ...
        if (!checkNodeName(NodeName)) {
            ...
            System.out.println("PurposeHeaderModel insertNodeRecord() circuit_name already exists in node table viz. - " + NodeName);
...
public int insertParentNodeRecord(String NodeName) {
        String query = "INSERT INTO circuit_info(circuit_name, generation_no, active, revision_no, remark) VALUES(?,?,?,?,?) ";
 ...
 return rowsAffected;
    ...
    public int getNodeId(String Node_name) {
        int node_id = 0;
        String query = "SELECT circuitinfo_id FROM circuit_info WHERE active='Y' and circuit_name = ? ";
        try {
            PreparedStatement pstmt = connection.prepareStatement(query);
            pstmt.setString(1, Node_name);
            ResultSet rset = pstmt.executeQuery();
            if (rset.next()) {
                node_id = rset.getInt("circuitinfo_id");
            }
        } catch (Exception e) {
    ...
    public int getNodeParentId(int Node_id) {
        ...
        String query = "SELECT circuitinfo_parent_id FROM tree where active='Y' and "
                + "circuitinfo_id = " + Node_id;
        try {
            PreparedStatement pstmt = connection.prepareStatement(query);
            //pstmt.setString(1, Node_name);
            ResultSet rset = pstmt.executeQuery();
            if (rset.next()) {
                node_parent_id = rset.getInt("circuitinfo_parent_id");
            }
        } catch (Exception e) {
    ...
    public String getNodeName(int Node_id) {
        String node_name = "";
        String query = "SELECT circuit_name FROM circuit_info WHERE active='Y' and circuitinfo_id = " + Node_id;
        try {
            ResultSet rset = connection.prepareStatement(query).executeQuery();
            if (rset.next()) {
                node_name = rset.getString("circuit_name");
            }
        } catch (Exception e) {
    ...
    public int getChildNodeParentId(int NodeChildId) {
        ...
        String query = "SELECT circuitinfo_parent_id FROM tree WHERE active='Y' and circuitinfo_id = " + NodeChildId;
        try {
            PreparedStatement pstmt = connection.prepareStatement(query);
            ResultSet rset = pstmt.executeQuery();
            if (rset.next()) {
                ChildNodeParentId = rset.getInt("circuitinfo_parent_id");
            }
        } catch (Exception e) {
    ...
    public int insertTreeRecord(int NodeId, int NodeParentId) {
        String query = "INSERT INTO tree(circuitinfo_id, circuitinfo_parent_id, circuitinfo_parent_rev_no, "
                + "tree_parent_id, revision, active, index_no, isSuperChild, circuitinfo_rev) "
                + "VALUES(?, ?, ?, ?, ?, ?, ?, ?,?) "; //tree_id,
    ...
    public int insertTreeRecord1(int NodeId, int NodeParentId,int index) {
        String query = "INSERT INTO tree(circuitinfo_id, circuitinfo_parent_id, circuitinfo_parent_rev_no, "
                + "tree_parent_id, revision, active, index_no, isSuperChild, circuitinfo_rev) "
                + "VALUES(?, ?, ?, ?, ?, ?, ?, ?,?) "; //tree_id,
        int rowsAffected = 0, Revision = 0;
        ...
        try {
            ...
            pstmt.setInt(5, Revision);
            pstmt.setString(6, "Y");
            pstmt.setString(7, String.valueOf(index));
            pstmt.setString(8, check_isSuperChild(NodeParentId, NodeId));
            pstmt.setInt(9, getNodeRev(NodeId));
            rowsAffected = pstmt.executeUpdate();
        } catch (Exception e) {
    ...
    public int insertParentRecord(String parentHeader, String childHeader, String description) {
        ...
        try {
            ...
            String query1 = "SELECT circuitinfo_id FROM tree where active='Y' and circuitinfo_parent_id = " + NodeChildId;
            try {
                ResultSet rset = connection.prepareStatement(query1).executeQuery();
                while (rset.next()) {
                    childList.add(rset.getInt("circuitinfo_id"));
                }

                for (int i = 0; i < childList.size(); i++) {
                    int NodeChildId1 = childList.get(i);
                    String query = "SELECT circuitinfo_id FROM tree where active='Y' and circuitinfo_parent_id = " + NodeChildId1;
                    ResultSet rs = connection.prepareStatement(query).executeQuery();
                    while (rs.next()) {
                        childList.add(rs.getInt("circuitinfo_id"));
                    }
                }
                ...
                for (int i = 0; i <= size; i++) {
                    ...
                    String query2 = "select max(revision_no) revision_no from circuit_info "
                            + "where active = 'Y' AND circuitinfo_id= " + NodeChildId;
                    String query3 = "INSERT INTO circuit_info(circuitinfo_id,circuit_name, generation_no, active, revision_no, remark) "
                            + " Select circuitinfo_id,circuit_name, ?, ?, ?, remark from circuit_info n where active='Y' and "
                            + " circuitinfo_id= " + NodeChildId;
                    String query4 = "Update node Set active = 'N' "
                            + "where circuitinfo_id= " + NodeChildId + " And revision_no = ?";
                    ...
                    String query5 = "select max(revision) revision from tree "
                            + "where active = 'Y' AND circuitinfo_id= " + NodeChildId;
                    String query6 = "INSERT INTO tree(tree_id,circuitinfo_id, circuitinfo_parent_id, circuitinfo_parent_rev_no, "
                            + "tree_parent_id, revision, active, index_no, isSuperChild, circuitinfo_rev) "
                            + " Select tree_id,?,?,?,?,?,?,?,isSuperChild,? "
                            + "from tree t where active='Y' and "
                            + " circuitinfo_id= " + NodeChildId;
                    String query7 = "Update tree Set active = 'N' "
                            + "where circuitinfo_id= " + NodeChildId + " And revision = ?";
    ...
    public int updateParentRecord(int NodeChildId, int NodeParentId) {
        int rowsaffected = 0;
        try {
            String query8 = "Update tree SET isSuperChild='N' where active='Y' and circuitinfo_id = " + NodeParentId;
    ...
    public int updatePrevParentRecord(int NodeChildId) {
        ...
        String query = "SELECT circuitinfo_parent_id FROM tree WHERE active = 'N' AND circuitinfo_id = " + NodeChildId;
        try {
            ResultSet rset = connection.prepareStatement(query).executeQuery();
            if (rset.next()) {
                node_Prev_parent_id = rset.getInt("circuitinfo_parent_id");
            }

            String query1 = "SELECT COUNT(*) FROM tree where active = 'Y' AND circuitinfo_parent_id = " + node_Prev_parent_id;
            ResultSet rst = connection.prepareStatement(query1).executeQuery();
            if (rst.next()) {
                count = Integer.parseInt(rst.getString(1));
            }
            if (count == 0) {
                String query2 = "Update tree SET isSuperChild='Y' where active='Y' and circuitinfo_id = " + node_Prev_parent_id;
    ...
    public int updatePrevGenIndex(int NodeChildId, int NodeChildGen, int NodeParentId, int NodeParentGen, String NodeChildIndex, int NodePrevParentId) {
        ...
        try {
            if (NodeChildGen == NodeParentGen) {
                String query = "SELECT COUNT(*) FROM circuit_info where active = 'Y' and generation_no = " + NodeChildGen;
                ResultSet rset = connection.prepareStatement(query).executeQuery();
                if (rset.next()) {
                ...
                if (count > 0) {    // not required
                    String query1 = "SELECT circuitinfo_id FROM circuit_info where active='Y' and generation_no = " + NodeChildGen;
                    ...
                    for (int i = 0; i < size; i++) {
                        ...
                        if (lastPart != count) {  // lastPart < count
                            ...
                            if (branchLastPart < lastPart) {
                                ...
                                String query2 = "Update tree SET index_no = ? where active='Y' and circuitinfo_id = " + branchNodeId;
                                PreparedStatement pstmt = connection.prepareStatement(query2);
                        ...
                        if (rowsAffected > 0) {
                            ...
                            for (int j = 0; j < childList1.size(); j++) {
                                ...
                                String query3 = "SELECT circuitinfo_id FROM tree where active='Y' and circuitinfo_parent_id = " + branchNodeId;
                                ResultSet rs1 = connection.prepareStatement(query3).executeQuery();
                                while (rs1.next()) {
                                    childList1.add(rs.getInt("circuitinfo_id"));
                                }
                            }
                            ...
                            for (int k = 0; k < (size1 - 1); k++) {
                                ...
                                String query4 = "SELECT index_no from tree where active='Y' and circuitinfo_id = (SELECT circuitinfo_parent_id FROM tree where active='Y' and "
                                        + "circuitinfo_id = " + branchNodeId1 + ")";
                                ...
                                String query5 = "Update tree SET index_no = ? where active='Y' and circuitinfo_id = " + branchNodeId1;
                ...
                String query = "SELECT COUNT(*) FROM tree where active='Y' and circuitinfo_parent_id = " + NodePrevParentId;
                ResultSet rset = connection.prepareStatement(query).executeQuery();
                if (rset.next()) {
                    count = Integer.parseInt(rset.getString(1));  // count>=2 always
                }
                count = count + 1; // not required
                if (count > 0) {    // not required
                    String query1 = "SELECT circuitinfo_id FROM tree where active='Y' and circuitinfo_parent_id = " + NodePrevParentId;
                    ResultSet rs = connection.prepareStatement(query1).executeQuery();
                    while (rs.next()) {
                        childList.add(rs.getInt("circuitinfo_id"));
                    }
                    ...
                    for (int i = 0; i < size; i++) {
                        ...
                        if (rowsAffected > 0) {
                            ...
                            for (int j = 0; j < childList1.size(); j++) {
                                ...
                                ResultSet rs1 = connection.prepareStatement(query3).executeQuery();
                                while (rs1.next()) {
                                    childList1.add(rs1.getInt("circuitinfo_id"));
                                }
                            }
    ...
    public String getNodeId_Gen(String parentHeaderName) { //int
        // int circuitinfo_parent_id = 0;
        String list = "";
        String query = "SELECT circuitinfo_id,generation_no FROM circuit_info WHERE active='Y' and circuit_name = ? ";
        try {
            PreparedStatement pstmt = connection.prepareStatement(query);
            pstmt.setString(1, parentHeaderName);
            ResultSet rset = pstmt.executeQuery();
            if (rset.next()) // node_parent_id = rset.getInt("circuitinfo_id");
            {
                list = rset.getInt("circuitinfo_id") + "," + rset.getString("generation_no");
            }
        } catch (Exception e) {
    ...
    public int getGenerationNo(int NodeParentId) {
        ...
        String query = "SELECT generation_no FROM circuit_info where active = 'Y' and circuitinfo_id = " + NodeParentId;
        try {
            ResultSet rset = connection.prepareStatement("SELECT COUNT(*) FROM circuit_info where active = 'Y' ").executeQuery();
            if (rset.next()) {
                count = Integer.parseInt(rset.getString(1));
            }
            if (count == 0) {
    ...
    public int getNodeParentRevNo(int NodeParentId) {
        int revision_no = 0;
        String query = "SELECT revision_no FROM circuit_info where active='Y' and circuitinfo_id = " + NodeParentId;
        try {
            ResultSet rst = connection.prepareStatement(query).executeQuery();
            rst.next();
    ...
    public int getGenNo() {
        int revision_no = 0;
        String query = "SELECT count(*) FROM circuit_info where active='Y' and generation_no = '0'";
        try {
            ResultSet rst = connection.prepareStatement(query).executeQuery();
            rst.next();
            revision_no = rst.getInt(1);
        } catch (Exception e) {
            System.out.println("PurposeHeaderModel getNodeParentRevNo() Error: " + e);
        }
        return revision_no;
    ...
    public int getNodeRev(int NodeId) {
        int revision_no = 0;
        String query = "SELECT revision_no FROM circuit_info where active='Y' and circuitinfo_id = " + NodeId;
        try {
            ResultSet rst = connection.prepareStatement(query).executeQuery();
            rst.next();
            revision_no = rst.getInt("revision_no");
    ...
    public int getNodeRev1(String NodeName) {
        int revision_no = 0;
        String query = "SELECT revision_no FROM circuit_info where active='Y' and circuit_name = ? ";
        try {
            PreparedStatement pstmt = connection.prepareStatement(query);
            pstmt.setString(1, NodeName);
    ...
    public int getTreeParentId(int NodeParentId) {
        ...
        String query = "SELECT tree_id FROM tree where active='Y' and circuitinfo_id = " + NodeParentId;
        try {
            ResultSet rset = connection.prepareStatement(query).executeQuery();
            rset.next();
            try {
    ...
    public String check_isSuperChild(int NodeParentId, int NodeId) {
        ...
        String query = "Update tree SET isSuperChild='N' where circuitinfo_id = " + NodeParentId;
        String query1 = "SELECT generation_no FROM circuit_info where circuitinfo_id = " + NodeId;
        String query2 = "SELECT max(generation_no) as generation_no FROM circuit_info";
//       try{
        try {
            PreparedStatement pstmt = connection.prepareStatement(query);
...
//        }
    ...
    public String getIndexNo(int NodeParentId, int NodeId) {
        ...
        int count = 0;
        String query = "SELECT COUNT(*) FROM tree where active='Y' and circuitinfo_parent_id = " + NodeParentId;
        String query1 = "SELECT max(CAST(index_no AS UNSIGNED)) as index_no from tree where active='Y' and circuitinfo_id IN(SELECT circuitinfo_id FROM circuit_info where active='Y' and "
                + "generation_no = (SELECT generation_no FROM circuit_info where active='Y' and circuitinfo_id = " + NodeId + "))";
        String query2 = "SELECT index_no FROM tree where active='Y' and circuitinfo_id = " + NodeParentId;
        String query3 = "SELECT index_no from tree where active='Y' and circuitinfo_id IN(SELECT circuitinfo_id FROM tree where active='Y' and "
                + "circuitinfo_parent_id = " + NodeParentId + ")";
        ...
        if (NodeParentId == 1) {
            try {
                ...
                System.out.println("PurposeHeaderModel getIndexNo() circuitinfo_parent_id is " + NodeParentId + " try block Error: " + e);
            }
        } else {
            try {
                ResultSet rtst = connection.prepareStatement(query2).executeQuery();
...
//       
        String query2 = "SELECT index_no FROM tree where active='Y' and circuitinfo_id = ? ";
...
//       
        try {
            ...
            System.out.println("PurposeHeaderModel getIndexNo_2() circuitinfo_parent_id is " + NodeParentId + " try block Error: " + e);
        }
//       }
    ...
    public String getChildIndex(int NodeId) {
        String index_no = "";
        String query = "SELECT index_no FROM tree where active='Y' and circuitinfo_id = " + NodeId;
        try {
            ResultSet rst = connection.prepareStatement(query).executeQuery();
            if (rst.next()) {
                index_no = rst.getString("index_no");
    ...
    public int getMaxTreeRev(int Node_id) {
        int maxRevision = 0;
        String query = "select max(revision) revision from tree where circuitinfo_id = " + Node_id;
        try {
            ResultSet rset = connection.prepareStatement(query).executeQuery();
            if (rset.next()) {
    ...
    public int deleteRecord(int NodeHeaderId) {
        ...
        String query = "UPDATE node SET active = 'N' WHERE active='Y' and circuitinfo_id = " + NodeHeaderId;
        maxRevision = getMaxTreeRev(NodeHeaderId);
        String query1 = "UPDATE tree SET active = 'N' and index_no = '0' WHERE active='Y' and revision = " + maxRevision + " and circuitinfo_id = " + NodeHeaderId;
        ...
        if (rowsAffected > 0) {
            System.out.println("Record deleted from circuit_info for circuitinfo_id = " + NodeHeaderId);
            ...
            try {
                if (rowsAffected1 > 0) {
                    System.out.println("Record deleted from tree for circuitinfo_id = " + NodeHeaderId);
                    /* code to update index_no of next generation -- starts */
                    List<Integer> childList1 = new ArrayList<Integer>();
                    childList1.add(NodeHeaderId);
                    for (int j = 0; j < childList1.size(); j++) {
                        NodeHeaderId = childList1.get(j);
                        String query3 = "SELECT circuitinfo_id FROM tree where active='Y' and circuitinfo_parent_id = " + NodeHeaderId;
                        ResultSet rs1 = connection.prepareStatement(query3).executeQuery();
                        while (rs1.next()) {
                            childList1.add(rs1.getInt("circuitinfo_id"));
                        }
...
//                             
    ...
    public int updateRecord(String NodeHeaderName, int NodeHeaderId) {
        ...
        try {
            if (!checkNodeName(NodeHeaderName)) {
                /* code for entry in node table -- starts here */
                String query2 = "select max(revision_no) revision_no from circuit_info "
                        + "where active = 'Y' AND circuitinfo_id= " + NodeHeaderId;
                String query3 = "INSERT INTO node(circuitinfo_id,circuit_name, generation_no, active, revision_no, remark) "
                        + " Select circuitinfo_id,?, generation_no, ?, ?, remark from circuit_info n where active='Y' and "
                        + " circuitinfo_id= " + NodeHeaderId;
                String query4 = "Update node Set active = 'N' "
                        + "where circuitinfo_id= " + NodeHeaderId + " And revision_no = ?";
                ...
                if (rowsaffected > 0) {
                    ...
                    if (rowsaffected1 > 0) {
                        ...
                        String query5 = "Update tree Set circuitinfo_rev = " + incRevision + " where active = 'Y' AND circuitinfo_id= " + NodeHeaderId;
                ...
                System.out.println("PurposeHeaderModel updateRecord() circuit_name already exists in node table viz. - " + NodeHeaderName);
    ...
    public List<String> getParent_Header_Name(String q) {
        ...
        int count = 0;
        // int nodeId=1;
        String query1 = "SELECT n.circuit_name "
                + " FROM circuit_info as n " //,tree as t
        ...
        try {
            ...
            q = q.trim();
            while (rset.next()) {
                String Parent_Header_Name = rset.getString("circuit_name");
        ...
        String query = "SELECT n.circuit_name "
                + "FROM node as n,tree as t, node as pn "
                + "where n.circuitinfo_id=t.circuitinfo_id AND t.circuitinfo_parent_id=pn.circuitinfo_id "
                + "And n.active='Y' And t.active='Y' and pn.active='Y' AND t.index_no NOT LIKE '0%' " //t.index_no NOT LIKE '%.0%' AND t.index_no NOT LIKE '0%'
                + "ORDER BY n.circuitinfo_id ";
    ...
    public List<String> getChild_Header_Name(String q, String parentHeaderName) {
        ...
        try {
            ...
            String query3 = "SELECT circuit_name from circuit_info As n where generation_no Between " + genNo + " And " + maxGenNo
                    + " and circuitinfo_id NOT IN (SELECT circuitinfo_id FROM tree where active='Y' AND  circuitinfo_parent_id = " + parentHeaderId + ") " // index_no LIKE '%.0%' AND index_no LIKE '0%' OR //AND index_no LIKE '%0%'
                    + " and active='Y' and circuitinfo_id != " + parentHeaderId;
            ...
            q = q.trim();
            while (rset.next()) {
                String childHeaderName = rset.getString("circuit_name");
    ...
    public List<String> getSearchIndex(String q) {
        List<String> list = new ArrayList<String>();
        //int nodeId=1;
        String query1 = "SELECT t.index_no " //,t.node_id,n.circuit_name, pn.node_name AS parent_node_name,t.active,t.remark
                + "FROM circuit_info as n,tree as t, circuit_info as pn "
                + "where n.circuitinfo_id=t.circuitinfo_id AND t.circuitinfo_parent_id=pn.circuitinfo_id "
                + "And n.active='Y' And t.active='Y' and pn.active='Y' AND t.index_no NOT LIKE '0%' " //t.index_no NOT LIKE '%.0%' AND t.index_no NOT LIKE '0%'
                + " ORDER BY CAST(SUBSTRING_INDEX(t.index_no, '.', 1) AS UNSIGNED),CAST(SUBSTRING_INDEX(t.index_no, '.', -1) AS UNSIGNED),t.index_no "; //ORDER BY t.index_no
    ...
    public List<String> getSearchSubHeader(String q) {
        List<String> list = new ArrayList<String>();
        String query1 = "SELECT n.circuit_name " //t.index_no,t.node_id,  pn.node_name AS parent_node_name,t.active,t.remark
                + "FROM circuit_info as n,tree as t, circuit_info as pn "
                + "where n.circuitinfo_id=t.circuitinfo_id AND t.circuitinfo_parent_id=pn.circuitinfo_id "
                + "And n.active='Y' And t.active='Y' and pn.active='Y' AND t.index_no NOT LIKE '0%' " //index_no not like '%0%'
                + " ORDER BY CAST(SUBSTRING_INDEX(t.index_no, '.', 1) AS UNSIGNED),CAST(SUBSTRING_INDEX(t.index_no, '.', -1) AS UNSIGNED),t.index_no "; //ORDER BY t.index_no
        ...
        try {
            ...
            q = q.trim();
            while (rset.next()) {
                String node_name = rset.getString("circuit_name");
    ...
    public List<String> getSearchParentHeader(String q) {
        List<String> list = new ArrayList<String>();
        String query1 = "SELECT pn.circuit_name AS parent_node_name " //t.index_no,t.node_id,n.node_name, t.active,t.remark
                + "FROM circuit_info as n,tree as t, circuit_info as pn "
                + "where n.circuitinfo_id=t.circuitinfo_id AND t.circuitinfo_parent_id=pn.circuitinfo_id "
                + "And n.active='Y' And t.active='Y' and pn.active='Y' AND t.index_no NOT LIKE '0%' " //index_no not like '%0%'
                + " ORDER BY CAST(SUBSTRING_INDEX(t.index_no, '.', 1) AS UNSIGNED),CAST(SUBSTRING_INDEX(t.index_no, '.', -1) AS UNSIGNED),t.index_no "; //ORDER BY t.index_no
    ...
    public ArrayList<CircuitInfoBean> getItemListForReport(String searchheader_index, String searchSub_header_name, String searchParent_header_name) {
        // String str[] = searchheader_index.split("\\.");
        String mainQuery = "", selectQuery; //subString = searchheader_index, tempString = "",  mainSNo = "1",
        PreparedStatement ps;
        ResultSet rs;
        ArrayList<CircuitInfoBean> mainList = new ArrayList<CircuitInfoBean>();
...
//            } else {
            mainQuery = "SELECT t.index_no,t.circuitinfo_id,n.circuit_name, pn.circuit_name AS parent_node_name,t.active,t.remark "
                    + "FROM circuit_info as n,tree as t, circuit_info as pn "
                    + "where n.circuitinfo_id=t.circuitinfo_id AND t.circuitinfo_parent_id=pn.circuitinfo_id "
                    + "And n.active='Y' And t.active='Y' and pn.active='Y' AND t.index_no NOT LIKE '0%' " //index_no not like '%0%'
                    + " AND if('" + searchSub_header_name + "'='', n.circuit_name LIKE '%%', n.circuit_name LIKE '%" + searchSub_header_name + "%')"
                    + " AND if('" + searchParent_header_name + "'='', pn.circuit_name LIKE '%%', pn.circuit_name LIKE '%" + searchParent_header_name + "%')"
            ...
            while (rs.next()) {
                ...
                headerNode.setNode_id(rs.getInt("circuitinfo_id"));
                headerNode.setTree_node_name(rs.getString("circuit_name"));
...
//            }
    ...
    public ArrayList<CircuitInfoBean> getItemListForReport_comment(String searchheader_index, String searchSub_header_name, String searchParent_header_name) {
        String str[] = searchheader_index.split("\\.");
        ...
        try {
            if (searchheader_index.isEmpty()) {
                ...
                mainQuery = "Select n.circuit_name, t.index_no "
                        + " from circuit_info as n, task as tk, tree as t "
                        + " where n.circuitinfo_id=tk.circuitinfo_id And n.circuitinfo_id=t.circuitinfo_id "
                        + " And n.active='Y' And tk.active='Y' And t.active='Y' AND t.index_no NOT LIKE '%0%' "
                        + " AND t.index_no = " + str[0];
                ps = connection.prepareStatement(mainQuery);
                rs = ps.executeQuery();
                while (rs.next()) {
                    CircuitInfoBean headerListReport = new CircuitInfoBean();
                    headerListReport.setIndex_no(rs.getString("index_no"));
                    headerListReport.setTree_node_name(rs.getString("circuit_name"));
                    name1.add(searchheader_index);
                    headerListReport.setName(name1);
                    try {
                        ArrayList<CircuitInfoBean> headerList = new ArrayList<CircuitInfoBean>();
                        for (int i = 0; i < str.length; i++) {
                            if (i == str.length - 1) {
                                ...
                                selectQuery = "Select n.circuit_name, t.index_no "
                                        + " from circuit_info as n, task as tk, tree as t "
                                        + " where n.circuitinfo_id=tk.circuitinfo_id And n.circuitinfo_id=t.circuitinfo_id "
                                        + " And n.active='Y' And tk.active='Y' And t.active='Y' AND t.index_no NOT LIKE '%0%' "
                            ...
                            rs = ps.executeQuery();
                            while (rs.next()) {
                                CircuitInfoBean itemHeader = new CircuitInfoBean();
                                itemHeader.setIndex_no(rs.getString("index_no"));
                                itemHeader.setTree_node_name(rs.getString("circuit_name"));
                    ...
                    try {
                        ArrayList<CircuitInfoBean> leafList = new ArrayList<CircuitInfoBean>();
                        String s = "";
                        ...
                        selectQuery = "SELECT t.index_no,t.circuitinfo_id,n.circuit_name, pn.circuit_name AS parent_node_name,t.active,t.remark "
                                + "FROM circuit_info as n,tree as t, circuit_info as pn "
                                + "where n.circuitinfo_id=t.circuitinfo_id AND t.circuitinfo_parent_id=pn.circuitinfo_id "
...
//                         ps.setDate(1, setDate(est_created_date));
                     /*   ps.setString(2, sNo);
                        ...
                        rs = ps.executeQuery();
                        while (rs.next()) {
                            CircuitInfoBean itemLeaf = new CircuitInfoBean();
                            itemLeaf.setIndex_no(rs.getString("index_no"));
                            itemLeaf.setTree_node_name(rs.getString("circuit_name"));
...
//                            itemLeaf.setRemark(rs.getString("description"));
    ...
    public ArrayList<CircuitInfoBean> getItemListFullForReport(String searchheader_index, String searchSub_header_name, String searchParent_header_name) {
        String str[] = searchheader_index.split("\\.");
        String subString = searchheader_index, tempString = "", mainSNo = "1", mainQuery = "", selectQuery;
        PreparedStatement ps;
        ResultSet rs, rsMain;
        ...
        try {
            if (searchheader_index.isEmpty()) {
                ...
                while (rsMain.next()) {
                    CircuitInfoBean itemListReport = new CircuitInfoBean();
                    itemListReport.setIndex_no(rsMain.getString("index_no"));
                    itemListReport.setTree_node_name(rsMain.getString("circuit_name"));
                    ...
                    try {
                        ...
                        selectQuery = "Select n.circuit_name, t.index_no , pn.circuit_name AS parent_node_name " // tk.amount,  tk.unit, tk.description
                                + " from circuit_info as n, task as tk, tree as t ,circuit_info as pn "
                                + " where n.circuitinfo_id=tk.circuitinfo_id And n.circuitinfo_id=t.circuitinfo_id AND t.circuitinfo_parent_id=pn.circuitinfo_id "
...
//                            itemLeaf.setRemark(rs.getString("description"));
    ...
    public byte[] generateHierarchyDetailPdfList(String jrxmlFilePath, ArrayList<CircuitInfoBean> finalMainList) {
        byte[] reportInbytes = null;
        HashMap mymap = new HashMap();
        try {
            ArrayList<CircuitInfoBean> dataList = finalMainList;
